pipeline {
	...
	stage('Build'){
	    steps {
		    sh "sudo docker build /home/ubuntu/jenkins/workspace/${JOB_NAME} -t shaiksaidhul/devopsdemo"
	   }
	}
	stage('Docker Push'){
		steps {
		    sh "sudo docker login --username shaiksaidhul --password ${dockerpass}"
                    sh "sudo docker push shaiksaidhul/devopsdemo:latest"
	        }
	}
